...
├── hooks/           # Custom React hooks (useAiChat.ts — AI chat state, IP rate limiting)
...
2. **Centralized Constants**: Declare constant readonly values in `src/constants/index.ts` instead of declaring hardcoded strings or design tokens in components. AI chat configuration (input limits, storage keys) is centralized under `AI_CHAT_CONFIG`.
...
6. **Custom Hooks** (`src/hooks/`):
   - `useAiChat` manages AI chat state including messages, model discovery, server-synchronized IP quota, and error handling.
   - Hooks must return a clean public API and keep internal state private.
   - Session persistence (model cache) uses `sessionStorage` with keys from `AI_CHAT_CONFIG`.
